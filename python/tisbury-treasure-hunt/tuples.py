...
def create_record(azara_record, rui_record):
    ...
    if compare_records(azara_record, rui_record):
        ...
        coord = get_coordinate(azara_record)
        location = rui_record[0]
        rui_coord = rui_record[1]
        quadrant = rui_record[2]
        return (treasure, coord, location, rui_coord, quadrant)
    else:
        return "not a match"
...
def clean_up(combined_record_group):
    ...
    for record in combined_record_group:
        treasure, coord, location, rui_coord, quadrant = record
        cleaned_records.append(f"('{treasure}', '{location}', {rui_coord}, '{quadrant}')")
