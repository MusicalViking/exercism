...
def categorize_dish(dish_name, dish_ingredients):
    ...
    if ingredient_set.issubset(OMNIVORE):
        return f"{dish_name}: OMNIVORE"
    # Fallback - shouldn't happen based on problem statement
...
def tag_special_ingredients(dish):
    ...
    ingredient_set = set(dish_ingredients)
    special_ingredients = SPECIAL_INGREDIENTS.intersection(ingredient_set)
...
def separate_appetizers(dishes, appetizers):
    ...
    result = [dish for dish in dishes if dish not in appetizer_set]
...
def singleton_ingredients(dishes, intersection):
    ...
    # Create union of all ingredients across all dishes
    all_ingredients = set()
    for dish in dishes:
        all_ingredients.update(dish)

    # Singleton ingredients are the symmetric difference between union and intersection
    # This gives ingredients that appear in the category but not in the intersection
    result_ingredients = all_ingredients ^ set(intersection)
